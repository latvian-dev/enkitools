...
import com.enkigaming.enkitools.cmd.*;
...
public class EnkiTools
{
	...
	public void preInit(FMLPreInitializationEvent e)
	{
		...
		mod = new LMMod(e, new EnkiToolsConfig(), null);
	...
	{
		EnkiToolsConfig.Login.loadStartingInv();
	...
	{
		if(w.provider.dimensionId != 0 || EnkiToolsConfig.WorldCategory.spawnDistance <= 0F) return false;
		double x = cx * 16D + 8.5D;
		double z = cz * 16D + 8.5D;
		double dist = EnkiToolsConfig.WorldCategory.spawnDistance;
	...
	{
		if(!EnkiToolsConfig.WorldCategory.enableWorldBorder) return false;
		
		int dist = EnkiToolsConfig.WorldCategory.getWorldBorder(w.provider.dimensionId);
		
		if(dist <= 0) return false;
		
		if(EnkiToolsConfig.WorldCategory.worldBorderAt0x0)
